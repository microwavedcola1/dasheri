...
impl MangoProgramTest {
    ...
    pub async fn start_new(config: &MangoProgramTestConfig) -> Self {
        ...
        test.add_program(
            "serum_dex",
            serum_program_id,
            processor!(process_serum_instruction),
        );
    ...
    pub async fn get_token_balance(&mut self, address: Pubkey) -> u64 {
        let token = self
            ...
            .unwrap()
            .unwrap();
        return spl_token::state::Account::unpack(&token.data[..])
            .unwrap()
            .amount;
    }
    ...
    pub async fn with_mango_account_deposit(
        ...
        let actual_mint_index = if mint_index == self.quote_index {
            QUOTE_INDEX
        } else {
            mint_index
        };
    ...
    pub async fn place_perp_order(
        ...
        .unwrap()];
        self.process_transaction(&instructions, Some(&[&user]))
            .await
            .unwrap();
    }
    ...
    pub async fn list_spot_market(&mut self, base_index: usize) -> SpotMarketCookie {
        ...
        let coin_vault = self
            .create_token_account(&vault_signer_pk, &coin_mint)
            .await;
        let pc_vault = self
            .create_token_account(&listing_keys.vault_signer_pk, &pc_mint)
            .await;
        ...
        self.process_transaction(&instructions, Some(&signers))
            .await
            .unwrap();
    ...
    pub async fn init_open_orders(&mut self) -> Pubkey {
        ...
        self.process_transaction(&instructions, Some(&[&orders_keypair]))
            .await
            .unwrap();
    ...
    pub async fn add_oracles_to_mango_group(&mut self, mango_group_pk: &Pubkey) -> Vec<Pubkey> {
        ...
        for _ in 0..self.quote_index {
            let oracle_pk = self
                .create_account(size_of::<StubOracle>(), &mango_program_id)
                .await;
            instructions.push(
    ...
    pub async fn perform_withdraw_with_delegate(
        ...
        .unwrap()];
        self.process_transaction(&instructions, Some(&[&delegate_user]))
            .await
    }
    ...
    pub async fn perform_liquidate_token_and_token(
        ...
        self.process_transaction(&instructions, Some(&[&liqor]))
            .await
            .unwrap();

        mango_group_cookie.mango_accounts[liqee_index].mango_account = self
            .load_account::<MangoAccount>(liqee_mango_account_pk)
            .await;

        mango_group_cookie.mango_accounts[liqor_index].mango_account = self
            .load_account::<MangoAccount>(liqor_mango_account_pk)
            .await;
    }
}
